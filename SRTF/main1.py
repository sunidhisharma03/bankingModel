...
passengers_attended = [0] * numTellers  # Counter to track number of passengers attended by each teller
    ...
    for customer_id in range(1, num_customers + 1):
        while not stop_simulation.is_set():
            if queue.qsize() < maxQueueSize:
                service_time = random.uniform(10, 20)  # Generate service time between 10 and 20 seconds
...
def teller_worker(queue, teller_id, waiting_times, turnaround_times, response_times):
    ...
    passengers_attended[teller_id - 1] = passengers_served  # Update number of passengers attended by this teller
    ...
    try:
        ...
        if waiting_times and turnaround_times and response_times:
            ...
            for i in range(numTellers):
                print(f"Teller {i+1} served {passengers_attended[i]} passengers.")
